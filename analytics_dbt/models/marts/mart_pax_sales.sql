...
from sales
         ...
         join {{ ref('dim_products') }} dp on dp.product_key = sales.product_key
...
group by df.flight_key, df.flight_number, dd.date, df.origin, df.destination,
         dd.year, dd.month, dd.day, dd.week_day, dd.weekday_name, dd.is_weekend,
         dt.day_period, dt.am_pm, dt.is_night, dp.item_id, dp.category
